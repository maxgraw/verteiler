...
describe('checkCapacity', () => {
    const groups = [makeGroup(0, 6, [0, 1, 2], -1), makeGroup(1, 5, [0, 1, 2], -1)];
...
describe('toUserMessage', () => {
    it('maps solver infeasibility to a capacity hint', () => {
        const message = toUserMessage(new Error('No feasible solution found (status: Infeasible).'));
        expect(message).toBe('Keine gültige Verteilung möglich. Prüfe ob die Kapazitäten ausreichen.');
    });
