...
describe('restore from localStorage', () => {
    ...
    it('restores a payload written by the current version', async () => {
        save({ version: VERSION, link: 'https://example.test/viewform', datum: '2026-05-04', uhrzeit: '18:00' });
        const state = await freshState();
...
describe('reset', () => {
    it('clears inputs and returns to the first step', async () => {
        ...
        state.parsedGroups = [{ id: 0, size: 1, members: 'Anna', choices: [0, 1, 2], currentSelection: -1 }];
